...
class KivyApp(App):
    ...
    def _press(self, btn):
        self.msg_out.send({'press': True, 'name': btn.text})
    ...
    def _release(self, btn):
        if btn.text == BTN_MOUNT:
            ...
            if self.loaded_path is not None:
                ...
                self.msg_out.send({'press': False, 'name': BTN_MOUNT, 'param': self.loaded_path})
        elif btn.text == BTN_UMOUNT:
            if self.loaded_path is not None:
                self.btn_mount = BTN_MOUNT
                self.msg_out.send({'press': False, 'name': BTN_UMOUNT, 'param': self.loaded_path})
                self.loaded_path = None
